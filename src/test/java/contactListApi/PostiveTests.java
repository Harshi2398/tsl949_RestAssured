...
public class PostiveTests {
	...
	public void gettingAllContact() {
		/*
		 * Given : Some Pre-condition When : user Performs some Operations Then Some
		 * post condition
		 */
		given().when().get("http://3.13.86.142:3000/contacts").then().log() // Print
				.body().statusCode(200); // Assertion
	}
	...
	public void addContact() {
		...
		id = given().header("Content-Type", "application/json").body(data.toJSONString()).when()
				.post("http://3.13.86.142:3000/contacts").then().log() // Print
				.body().statusCode(200) // Assertion
				.extract().path("_id");
	...
	public void getSpecificContact() {
		...
		given().when().get("http://3.13.86.142:3000/contacts/" + id).then().log() // Print
				.body().statusCode(200); // Assertion
	}
	...
	public void updateContact() {
		...
		given().header("Content-Type", "application/json").body(data.toJSONString()).when()
				.put("http://3.13.86.142:3000/contacts/" + id).then().log() // Print
				.body().statusCode(204); // Assertion
	}

	@Test(enabled = true, dependsOnMethods = "updateContact", description = "Get Specific Contact")
	public void deleteContact() {
		System.out.println("Deleting Contact");
		given().when().delete("http://3.13.86.142:3000/contacts/" + id).then().log() // Print
				.body().statusCode(204); // Assertion
	}
